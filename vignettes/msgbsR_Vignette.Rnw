...
Current GBS data analysis tools do not fulfil all experimental designs. For example, genotyping by sequencing (GBS) experiments using methylation sensitive restriction enzyme (REs), which is also known as methylation sensitive genotyping by sequencing (MS-GBS), is an effective method to identify differentially methylated sites that may not be accessible in other technologies such as microarrays and capture sequencing. However, current data analysis tools do not satisfy the requirements for these types of experimental designs.
...
my_path <- system.file("extdata", "T1.bam", package = 'msgbsR')
my_path <- gsub('T1.bam', '', my_path)
...
After the table of read counts has been generated into the R environment, the next step would be to confirm that the cut sites were the correctly generated sites. In this example, the restriction enzyme that has been used is MspI which recognizes a 4bp sequence (CCGG).
...
msgbsR offer two approaches to checking the cut sites. The first approach is to use a BSgenome which can be obtained from Bioconductor. In this example, BSgenome.Hsapiens.UCSC.hg38 will be used.

<<run checkCuts with BSgenome>>=
library(BSgenome.Hsapiens.UCSC.hg38)

correctCuts <- checkCuts(cutSites = x, cutIDs = row.names(datCounts),
                          BSgenome = Hsapiens, seq = 'CCGG')
@

If a BSgenome is unavailable, another option to checking the cut sites is to use a fasta file. msgbsR comes with the fasta file for chromosome 22 from UCSC hg38. To use the checkCuts function with a fasta file simply leave the BSgenome input as blank (NULL) and input the path to the fasta file location. An example of this is shown below.

<<run checkCuts>>=
chr22 <- system.file("extdata", "chr22.fa.gz", package = 'msgbsR')
correctCuts <- checkCuts(cutSites = x, cutIDs = row.names(datCounts),
                         fastaPath = chr22, seq = 'CCGG')
@
...
y <- data.frame(c(rep('First Trimester', 3), rep('Term', 3)))
colnames(y) <- 'Group'
...
msgbsR utilizes edgeR in order to determine which cut sites are differentially methylated between groups. Since MS-GBS experiments can have multiple groups or conditions msgbsR offers a wrapper function of edgeR (Zhou et al., 2014) tools to automate differential methylation analysis.
...
top <- diffMeth(countMatrix = datCounts, pd = y, cateogory = 'Group',
                condition1 = 'First Trimester', condition2 = 'Term',
                block = NULL,
                cpmThreshold = 1, thresholdSamples = 2)
@

The top object now contains a data frame of the cut sites that had a CPM > 1 in at least 2 samples and which cut sites are differentially methylated between the two groups.

\section{Visualization of cut site locations}

The msgbsR package contains 2 functions to allow visualization of the location of the cut sites. Given the lengths of the chromosomes the cut sites can be visualized in a circos plot (Figure 2) or a karyogram (Figure 3). For example, differentially methylated sites can be visualized in a circos plot or a karyogram.

Firstly, define the length of the chromosome.
<<chr22 length>>=
chr22 <- matrix(c('chr22', '50818468'), nrow = 1, ncol = 2)
@
...
<<circos plot>>=
plotCircos(cutSites = z[,c(1,3,3)], genome = chr22,
...
<<karyogram plot>>=
plotChr(cutSites = z[,c(1,3,3)], genome = chr22)
@
...
@
\end{center}
\caption{A circos plot of chromosome 22 representing the top 100 differential methylated cut sites.}
\label{fig:fig2}
\end{figure}
...
@
\end{center}
\caption{A karyogram of chromosome 22 showing the location of differential methylated cut sites.}
\label{fig:fig3}
\end{figure}
...
<<annotation>>=
mygff <- system.file("extdata", "chr22.gff3", package = 'msgbsR')
z[ ,1] <- gsub('chr', '', z[,1])
